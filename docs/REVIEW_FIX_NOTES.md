...
- **已执行完成**：从原 273 只扩展到 **1809 只**（全 universe 覆盖），28724 行季度数据，范围 2022Q1 ~ 2026Q1，ROE 覆盖率 95.7%。
...
### 补齐基本面后的 value profile 回测（2025 全年 rolling）

基本面数据从 273 只补齐到 1809 只后，value profile（依赖 PE/PB/ROE）终于有完整数据可用：

| 指标 | balanced（补齐前） | value（补齐后） |
|------|-------------------|-----------------|
| 总收益 | -9.75% | -24.57% |
| 年化收益 | -10.10% | -25.35% |
| Sharpe | -0.120 | -0.729 |
| 最大回撤 | -29.84% | -36.57% |
| RankIC | -0.0227 | -0.0213 |

**解读**：value profile 在 2025 年跑输 balanced 是符合预期的——2025 年是 AI/成长股主线行情（DeepSeek 概念、科技板块领涨），低估值 value 策略天然逆风。RankIC 仍为负（-0.0213）说明模型选股能力本身不足，这与数据补齐无关，需要更好的因子组合或 regime-conditional 模型。

**注意**：balanced 回测因与 value 回测并行触发 DuckDB 跨进程锁失败（DuckDB 不支持多进程同时写同一库）。`_WRITE_LOCK` 只保护进程内并发，跨进程仍需串行运行回测。
