...
- ✅ 基本面因子工程（PE / PB / ROE）
- ✅ 宏观因子工程（CPI / PMI / M2）
- ✅ 核心层接口抽象（BaseModel / BaseStrategy / BaseDataSource / BaseBroker）
- ✅ LightGBM 选股模型
...
- ✅ 网站已通过 `npm run check` 与 `npm run build`
...
├── backtest/           # 回测引擎 + 绩效指标
├── cli/                # 命令行入口
├── web/                # 前后端网站（React + Hono + tRPC + Drizzle + MySQL）
...
| 问题 | 说明 | 建议修复方向 |
|------|------|-------------|
| 数据量小 | 当前已入库 49 只上证 50 成分股（约 21,942 条日线），1 只无数据 | 继续跑 `data-update` 完成剩余数据；可扩展全 A 股 |
