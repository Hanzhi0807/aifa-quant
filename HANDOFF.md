...
- ✅ 模拟交易（Paper Trading）：加载模型、生成信号、`SimulatedBroker` 虚拟成交、DuckDB 持久化，详见 `docs/PAPER_TRADING.md`
- ✅ CLI 入口（含 `paper-trade` 子命令）
...
**默认用 AkShare（免费，无需 token）**

```bash
# 全量沪深 300（2023-2024，顺序下载，需数分钟）
python -m aifa_quant.cli.main data-update \
  --universe 沪深300 \
  --start 20230101 --end 20241231

# 同时缓存基本面和宏观数据（需要 iFind token）
python -m aifa_quant.cli.main data-update \
  --universe 沪深300 \
  --start 20230101 --end 20241231 \
  --fundamental --macro
...
# 只测试前 5 只
python -m aifa_quant.cli.main data-update \
  --universe 沪深300 --sample 5 \
  --start 20240101 --end 20241231
```

**或用 iFind MCP（需要 token 和额度）**
...
### 5.6 模拟交易（Paper Trading）

模拟交易默认使用 DuckDB 中**最新缓存的交易日**作为“今天”，离线生成信号、模拟下单，不消耗 iFind 额度。

```bash
# 初始化账户（会清空之前的模拟状态）
python -m aifa_quant.cli.main paper-trade reset --cash 1000000

# dry-run：只打印选股信号和计划交易，不写入数据库
...
- `paper_orders` - 订单明细（symbol, side, quantity, fill_price, commission, stamp_duty, status）
- `paper_nav` - 每日净值（cash, market_value, total_value）
