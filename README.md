...
| 维度 | 说明 |
|------|------|
| **开箱即用** | 提供完整的 Web 仪表盘（React + tRPC），本地 `npm run dev` 即可查看选股结果与权益曲线；同时支持一键部署到 Vercel，零运维即可在手机上随时查看持仓信号。适合没有量化基础的投资者快速上手。 |
| **按日更新，非实时** | 系统设计为**日频策略**——每个交易日收盘后增量更新行情、重新打分、生成次日调仓信号。不追求分钟级/Tick 级响应，因此**不适合日内交易或盯盘型策略**，但对中长线轮动投资者来说足够且稳定。 |
| **多层风控** | ATR 止损/止盈/暴跌/利润回撤四道防线 + 市场体制过滤（熊市自动暂停建仓） + 行业集中度上限（避免单板块过度暴露） + 波动率仓位 sizing。宁可少赚，不多亏。 |
| **高度可扩展** | 因子层面：技术面 / Alpha101 / 基本面 / 宏观 / 情绪已内置，还可自行接入 KOL 观点蒸馏、另类数据、舆情 NLP 等自定义因子。模型层面：LightGBM / LambdaRank / Ensemble / XGBoost 均已集成，新增模型只需实现 `BaseModel` 接口。策略层面：`TopKDropoutStrategy` 为默认模板，可继承并覆写选股逻辑。 |
| **数据安全** | 所有行情、持仓、模型文件均保存在本地 DuckDB，**不上传 GitHub**；`.env` 中的 token 始终在 `.gitignore` 中；Web 仪表盘仅暴露只读查询接口，不涉及任何交易指令。 |
| **透明可审计** | SHAP 可解释性分析让你清楚模型在"看"什么因子；每周自动生成的 AI 选股报告给出完整推理链条，而非黑箱输出。 |
...
- **选股增强**：行业集中度控制（防止单板块暴露）+ 市场体制过滤（熊市暂停建仓）+ Dropout 滞后（减少换手）。
...
| Profile | 名称 | 持仓数 | 行业上限 | 熊市过滤 | 风格 |
|---------|------|--------|----------|----------|------|
| `aggressive` | 激进型 | 15 | 35% | 关闭 | 高集中度，追求超额收益 |
| `balanced` | 均衡型 | 20 | 25% | MA < 0.95 | 攻守兼备，适合大多数人 |
| `conservative` | 稳健型 | 30 | 20% | MA < 0.97 | 充分分散，严控回撤 |
| `growth` | 成长型 | 20 | 30% | MA < 0.95 | 聚焦高成长潜力股 |
| `value` | 价值型 | 25 | 25% | MA < 0.93 | 低估值选股，安全边际优先 |

### 选股增强

- **行业集中度控制**：`max_industry_pct` 限制单一行业占比，贪心选股时跳过超限行业，防止板块集中暴露。
- **市场体制过滤**：`regime_ma_threshold` 通过沪深 300 的 MA20/MA60 比值判断大盘状态，在严重下跌趋势中暂停新建仓。
- **Dropout 滞后**：已持有股票排名只要不掉出 `top_k × 2`，不主动卖出，减少高频换手。
...
- 熊市体制过滤：MA20/MA60 < threshold 时暂停新建仓。
