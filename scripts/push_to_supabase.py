...
Reads current positions and recent orders from DuckDB, pushes to Supabase
for the web dashboard. No intermediate CSV files needed.
...
def load_positions_from_duckdb(store: DuckDBStore, profile: str) -> pd.DataFrame:
    ...
    cols = [r[0] for r in store.conn.execute("DESCRIBE paper_positions").fetchall()]
...
def load_recent_orders(store: DuckDBStore, profile: str, days: int = 5) -> pd.DataFrame:
    ...
    cols = [r[0] for r in store.conn.execute("DESCRIBE paper_orders").fetchall()]
    has_profile = "profile" in cols
...
def push_signals(client, positions: pd.DataFrame, trade_date: str, profile: str):
    """Push current positions as daily signals (top-ranked stocks)."""
    if positions.empty:
...
def push_portfolio(client, positions: pd.DataFrame, trade_date: str, profile: str):
...
def main():
    ...
    for profile in profiles:
        ...
        if positions.empty:
            ...
            continue
        print(f"[{profile}] Found {len(positions)} positions")
